...
export const TeamCarousel: React.FC<TeamCarouselProps> = ({
  ...
  const paginate = useCallback(
    (newDirection: number, isUserAction = false) => {
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      if (pauseOnHover && autoPlay > 0 && !userInteracted) {
        interval = setInterval(() => {
          paginate(1);
        }, autoPlay);
      }
  ...
  }, [autoPlay, paginate, pauseOnHover, userInteracted]);
        ...
        {showArrows && (
          <>
            <motion.button
              ...
              className="absolute left-5 top-1/2 transform -translate-y-1/2 bg-primary/80 hover:bg-primary text-white w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110"
              whileTap={{ scale: 0.9 }}
            >
            ...
            <motion.button
              onClick={() => paginate(1, true)}
              className="absolute right-5 top-1/2 transform -translate-y-1/2 bg-primary/80 hover:bg-primary text-white w-10 h-10 rounded-full flex items-center justify-center z-20 transition-all duration-300 hover:scale-110"
              whileTap={{ scale: 0.9 }}
            >
                  ...
                  {infoPosition === "overlay" && (
                    <div
                      ...
                      style={{
                        background:
                          infoBackground ||
                          "linear-gradient(transparent, rgba(128, 0, 0, 0.9))",
                        color: infoTextColor,
                      }}
                    >
      ...
      {infoPosition === "bottom" && members[currentIndex] && (
        ...
        >
          ...
          {members[currentIndex].bio && (
            <div className="text-base mt-4 max-w-lg mx-auto opacity-90">
              {members[currentIndex].bio.split(" • ").map((part, index) => {
                ...
                if (emailMatch) {
                  return (
                    <a
                      ...
                      className="inline-flex items-center gap-2 text-base font-medium transition-all duration-300 hover:gap-3 underline decoration-1 underline-offset-4 hover:decoration-2 hover:scale-105"
                      style={{ color: "#FF8A8A" }}
                ...
                return (
                  <span key={index} style={{ color: infoTextColor }}>
                    {part}
                    {index <
                      members[currentIndex].bio!.split(" • ").length - 1 &&
                      " • "}
                  </span>
                );
              })}
